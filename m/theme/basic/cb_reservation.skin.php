...
<section class="rsrv_nav_section">
  ...
  <form name="buyform" id="buyform" method="post" action="<?php echo $order_action_url; ?>"  autocomplete="off">
    ...
    <div class="reservation_check_info">
      ...
      <div class="auto_side">
        <div class="shadow_box">
          <h4>대여시간</h4>
          <h2><?php echo $str_in_date.$in_yoil;?></h2>
          <h3>15:00</h3>
        </div>
        <div class="shadow_box">
          <h4>반납시간</h4>
          <h2><?php echo $str_out_date.$out_yoil;?></h2>
          <h3>15:00</h3>
        </div>
      </div>
      <h1 class="time shadow_box">24시간</h1>
      <?php } ?>
  ...
  <section class="driver_info_section">
    <div class="rsrv_info_div">
      ...
      <small>운전자 추가 1인은 별도의 추가비용 없이 등록 가능합니다.</small>
  ...
  <section class="payment_info_section">
    ...
    <div class="total_payment auto_side">
      <h2>결제금액</h2>
      <span>
        <input type="text" name="tot_price" id="tot_price" value="<?php echo display_price($ori_price); ?>" class="price_bold" style="outline:0;text-align:right;border:0;" readonly /> 원
      </span>
    </div>
  </section>
  ...
  <section class="payment_method_section">
    <h1>결제수단</h1>
    <ul>
      <li><label class="lb_check" id="card_lb"><input type="radio" name="pay_method" id="card" value="간편신용카드" <?php if($member['pay_method'] == '간편신용카드' || $member['pay_method'] == '') { ?> checked <?php } ?> /><img src="../img/payment-method.png" alt="신용카드(간편)" />신용카드(간편)</label></li>
      <li><label id="ccard_lb"><input type="radio" name="pay_method" id="credit_card" value="신용카드" <?php if($member['pay_method'] == '신용카드') { ?> checked <?php } ?>/><img src="../img/credit_card.png" alt="신용카드(일반)" />신용카드(일반)</label></li>
      <li><label id="bank_lb"><input type="radio" name="pay_method" id="simple_bank" value="간편계좌" <?php if($member['pay_method'] == '간편계좌') { ?> checked <?php } ?>/><img src="../img/online-banking.png" alt="간편계좌" />간편계좌 결제</label></li>
  ...
  <section class="payment_btn_section">
    ...
    <div class="submit_btn_div">
      <button type="button" id="res_submit_btn"><?php echo display_price($ori_price);?> 결제하기</button>
    </div>
  </section>
      ...
      <div class="modal-content">
        <div class="modal-body">
          ...
          <ul class="coupon_radio_ul">
            ...
            ?>
            <li class="shadow_box">
              <label class="auto_side coupon_label" for="coupon<?php echo $i; ?>">
                <div>
                ...
                <div>
                  <h4><? echo $row['cp_subject']; ?></h4>
                  <small><? echo $inv_date; ?> 사용가능</small>
                </div>
                <div>
                  <h4><? echo $sale_amt; ?> 할인</h4>
                </div>
              </label>
            </li>
            <?php } ?>
...
function fbuyform_submit(f){
  ...
  if(window.MyApp && window.MyApp.callMessage){
    ...
    intent.putExtra("Moid", "moid1234567890");
    intent.putExtra("GoodsName", "테스트상품");
    intent.putExtra("Amt", "5000");
    intent.putExtra("DutyFreeAmt", "0");
    intent.putExtra("PayMethod", "CARD");
    intent.putExtra("GoodsCnt", "1");
    intent.putExtra("BuyerName", "테스트");
    intent.putExtra("MallUserID", "test001");
    intent.putExtra("BuyerTel", "01012341234");
    intent.putExtra("BuyerEmail", "test@infinisoft.co.kr");
    */
    // 카드 & 간편계좌 & 통합페이

      if (PayMethod == "CARD" || PayMethod == "EBANK" || PayMethod == "EPAY" ) {
